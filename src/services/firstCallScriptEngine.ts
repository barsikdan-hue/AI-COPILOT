...
function sanitizePaymentMethodUncertainty(
  ...
  for (const turn of turns) {
    ...
    const explicitUncertainty = /(?:не\s+(?:знаю|решил\p{L}*|определил\p{L}*|выбрал\p{L}*)|сомнева\p{L}*|дума\p{L}*[^.!?]{0,40}(?:надо|нужно)\s+ли|(?:надо|нужно)\s+ли[^.!?]{0,35}ипотек|ипотек\p{L}*[^.!?]{0,45}или\s+не\s+(?:надо|нужно|брать|использовать))/iu.test(text);
    const schemeNotChosen = /(?:схем\p{L}*|вариант\p{L}*)[^.!?]{0,60}(?:пока\s+)?не\s+(?:выбрал\p{L}*|выбран\p{L}*|определил\p{L}*|определен\p{L}*|определён\p{L}*|решил\p{L}*)|(?:окончательн\p{L}*|пока)[^.!?]{0,45}(?:схем\p{L}*|вариант\p{L}*)[^.!?]{0,45}не\s+(?:выбрал\p{L}*|выбран\p{L}*|определил\p{L}*|определен\p{L}*|определён\p{L}*|решил\p{L}*)/iu.test(text);
    const alternativeChoice = /(?:либо|или)[^.!?]{0,35}ипотек\p{L}*[^.!?]{0,45}(?:либо|или)[^.!?]{0,35}рассроч\p{L}*|ипотек\p{L}*[^.!?]{0,45}(?:либо|или|и)[^.!?]{0,35}рассроч\p{L}*|сравнива\p{L}*[^.!?]{0,45}ипотек\p{L}*[^.!?]{0,35}(?:и|или|либо)[^.!?]{0,35}рассроч\p{L}*/iu.test(text);
    const uncertain = explicitUncertainty || schemeNotChosen || (alternativeChoice && /(?:возможн\p{L}*|рассматрива\p{L}*|сравнива\p{L}*|пока|схем\p{L}*|вариант\p{L}*)/iu.test(text));
...
function sanitizeDownPaymentWithoutAmount(
  ...
  for (const turn of turns) {
    ...
    const hasFunds = /(?:часть\s+средств|средств\p{L}*|деньг\p{L}*)[^.!?]{0,80}(?:есть|доступн\p{L}*|на\s+руках)|(?:есть|доступн\p{L}*)[^.!?]{0,60}(?:часть\s+средств|средств\p{L}*|деньг\p{L}*)/iu.test(text);
    const amountNearDownPayment = /(?:первоначальн\p{L}*|перв\p{L}*)\s+взнос[^.!?]{0,35}\d+(?:[.,]\d+)?\s*(?:млн|миллион\p{L}*|тыс\p{L}*|%|руб)|\d+(?:[.,]\d+)?\s*(?:млн|миллион\p{L}*|тыс\p{L}*|%|руб)[^.!?]{0,35}(?:первоначальн\p{L}*|перв\p{L}*)\s+взнос/iu.test(text);
    const explicitlyNoAmount = /без\s+конкретн\p{L}*\s+сумм|сумм\p{L}*\s+(?:пока\s+)?не\s+(?:называл\p{L}*|говорил\p{L}*)/iu.test(text);
    const hasAmount = amountNearDownPayment && !explicitlyNoAmount;
