...
const isDeferredGoalValue = (value: unknown): boolean => {
  const text = normalize(typeof value === 'string' ? value : '');
  return /^(?:не\s+определен\p{L}*|не\s+решен\p{L}*|пока\s+не\s+(?:решил\p{L}*|определил\p{L}*|знаю))/iu.test(text);
};
...
export function chooseDialoguePolicyTarget(
  ...
  const concretePastExperienceKnown = turns.some(
    (turn) => turn.speaker === 'client' && clientHasConcretePastExperience(normalize(turn.text))
  );
  ...
  const goalDeferred =
    goalHasDeferredValue ||
    (!goalKnown && clientDefersGoal(latest, latestAgent));

  const latestAnswersPastExperience =
    pastExperienceQuestionPattern.test(latestAgent) &&
  ...
  if (
    ...
    goalDeferred &&
    !experienceClosed &&
    !experienceHintDismissed
  ) {
    decisions.push(candidate(
      'experience',
      'experience',
      'ask_experience',
      'Клиент пока не определил сценарий покупки. Вместо абстрактных критериев сначала выясняем реальный опыт: что уже смотрел и что понравилось или не подошло.',
      89,
    ));
  }
  if (
    ...
    decisions.push(candidate(
      ...
      'criteria',
      'ask_criteria',
      'Цель пока не определена, но реальный опыт поиска уже разобран. Теперь можно мягко выделить критерии без повторного вопроса о цели.',
      87,
    ));
  }
  if (
  ...
  if (
    ...
    !clientHasNoConcreteExperience(latest) &&
    !agentAsked(turns, pastExperienceQuestionPattern)
  ) {
