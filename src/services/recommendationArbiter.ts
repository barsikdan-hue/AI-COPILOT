...
export function arbitrateRecommendationCandidates(
  ...
  const sameMeaning = Boolean(current.semanticKey && best.semanticKey && current.semanticKey === best.semanticKey);
  if (sameMeaning) return { winner: current, ranked };

  const hardSupersede = best.suppressesLowerPriority || best.actionType === 'RESPECT_STOP';
  if (hardSupersede || best.score >= currentScore + supersedeMargin) return { winner: best, ranked };
