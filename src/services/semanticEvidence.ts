...
export function extractSemanticCriteria(text: string): SemanticCriterion[] {
  ...
  addUniqueCriterion(
    ...
    firstMatch(
      lower,
      /(?:инфраструктур\p{L}*|логистик\p{L}*|транспортн\p{L}+\s+доступност\p{L}*|не\s+(?:хочу\s+)?(?:быть\s+)?отрезан\p{L}*\s+от\s+цивилизаци\p{L}*|не\s+(?:хочу\s+)?(?:быть\s+)?вырезан\p{L}*\s+из\s+социум\p{L}*|(?:важн\p{L}*|нужн\p{L}*|хоч\p{L}*|критери\p{L}*)[^.!?]{0,35}(?:рестораны?|магазины?|кафе|спа|бассейн\p{L}*|школ\p{L}*|детск\p{L}+\s+сад\p{L}*))/iu,
    ),
  );
  addUniqueCriterion(
    out,
  ...
  addUniqueCriterion(
    ...
    firstMatch(
      lower,
      /(?:(?:хоч\p{L}*|нужн\p{L}*|важн\p{L}*|ценю|предпочита\p{L}*)[^.!?]{0,24}тишин\p{L}*|тишин\p{L}*[^.!?]{0,24}(?:важн\p{L}*|нужн\p{L}*|хоч\p{L}*|предпочита\p{L}*)|(?:больше|скорее)\s+тишин\p{L}*|тишин\p{L}*\s*,?\s*но|тих\p{L}+\s+(?:мест|район|двор)|спокойн\p{L}+\s+(?:мест|район|окруж)|без\s+шум\p{L}*)/iu,
    ),
  );
  addUniqueCriterion(
    out,
